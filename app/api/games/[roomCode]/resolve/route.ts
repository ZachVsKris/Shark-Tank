...
export async function POST(request:NextRequest,context:{params:Promise<{roomCode:string}>}){try{
  ...
  for(let slot=0;slot<ids.length;slot++){
    ...
    const valid=r2.filter(b=>!b.passed).map(b=>({...b,distance:dealDistance(Number(b.cash),Number(b.equity),company.actualDealCash,company.actualDealEquity)})).sort((a,b)=>a.distance-b.distance||new Date(a.created_at).getTime()-new Date(b.created_at).getTime());if(!valid.length)continue;
